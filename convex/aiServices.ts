...
// ═══════════════════════════════════════════════════════════════════════════
// GENERATE AND STORE VOICE PREVIEW (from sorah — MiniMax TTS)
// Called by users.ts completeOnboarding
// ═══════════════════════════════════════════════════════════════════════════
export const generateAndStoreVoicePreview = action({
  args: { voiceId: v.string() },
  handler: async (ctx, { voiceId }): Promise<{ success: boolean; storageId?: Id<"_storage">; error?: string }> => {
    console.log("[generateAndStoreVoicePreview] generating for:", voiceId);
    try {
      const apiKey = process.env.MINIMAX_API_KEY;
      if (!apiKey) throw new Error("MINIMAX_API_KEY not set");
      ...
      console.log("[generateAndStoreVoicePreview] stored:", storageId);
